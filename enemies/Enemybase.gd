...
signal died(enemy)
signal activated(enemy)
signal deactivated(enemy)
...
func _die() -> void:
	...
	if audio:
		match _last_hit_source:
			"circle":
				audio.play_enemy_die(audio.snd_enemy_die_circle)
			_:
				audio.play_enemy_die(audio.snd_enemy_die_default)
	...
	if pool_owner:
		pool_owner.return_object(pool_scene, self)  # ✅ ONLY ONCE
		
func activate(spawn_pos: Vector2, target: CharacterBody2D):
	_is_dead = false
	activated.emit(self)
